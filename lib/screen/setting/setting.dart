...
//            ),
            ...
            Consumer<UserModel>(
                builder: (BuildContext context,UserModel value,Widget child){
                  code  = AppGlobal.profile.user.invite_code;
                  return
                    InkWell(
                        onTap: () {
            ...
            Consumer<UserModel>(
                builder: (BuildContext context,UserModel value,Widget child){
                  level  = AppGlobal.profile.user.level;
                  return
                    InkWell(
                        onTap: () {
                        },
            ...
            Consumer<UserModel>(
                builder: (BuildContext context,UserModel value,Widget child){
                  ...
                  return
                    InkWell(
                        onTap: () {
//                          Navigator.of(context).push(PageRouteBuilder(pageBuilder: (_,__,___)=> new ChangePassword()));
                          Navigator.push(
                              context,
                              MaterialPageRoute(builder: (context) => ChangePassword()));
                        },
                        child: listSetting("修改密码", level));
                }
            ),
            ///
                                      ...
                                      {'text': '确定','background': Color(0xff407fff), 'onPressed':(){
                                        ...
                                        apiClient.changeName(_temp,AppGlobal.profile.token).then((res){
                                          ...
                                          if( res.data['err'] != true ){
                                            Provider.of<UserModel>(context, listen: false).changeName(res.data['data']['user_name']);
                                          }
                                        });
                                        Navigator.of(context).pop();
                                      }},
          ...
          child: Column(
            ...
            children: <Widget>[
              ClipRRect(
                ...
                child: Image.network('https://darasakor.oss-cn-chengdu.aliyuncs.com/qrcode.png',
                    fit: BoxFit.cover),
              ),
              Container(
                margin: EdgeInsets.only(top: 5),
                child: Text('将你的推广码分享给你的会员',style: TextStyle(
                    fontSize: 13,
                    letterSpacing: 1,
